...
$this->title = 'Berkas Masuk Diterima';
...
<div class="main-content-container overflow-hidden">
    ...
    <div class="card bg-white border-0 rounded-3 mb-4 shadow-sm">
        <div class="card-body p-4">
            ...
            <h5 class="card-title fw-semibold mb-3 border-bottom pb-2">Inbox Diterima</h5>

            <div class="d-flex justify-content-start align-items-center flex-wrap gap-2 mb-4">
                <?= Html::beginForm(['index'], 'get', [
                    'id' => 'form-search-diterima',
                        ...
                        <tbody>
                            <?php if ($dataProvider->totalCount > 0): ?>
                                <?php foreach ($dataProvider->getModels() as $index => $model): ?>
                                    <tr>
                                        <td class="text-center text-secondary"><?= $dataProvider->pagination->page * $dataProvider->pagination->pageSize + $index + 1 ?></td>
                                        <td class="text-center">
                                            <a href="#"><i class="material-symbols-outlined fs-18 text-info text-primary">history</i></a>
                                        </td>
                                        <td class="text-center text-primary"><?= Html::encode($model['nomor_daftar']) ?></td>
                                        <td><?= Html::encode($model['nama_izin']) ?></td>
                                        <td class="text-center"><span class=""><?= Html::encode($model['jenis_permohonan']) ?></span></td>
                                        ...
                                        <td class="text-center">
                                            <div class="d-flex gap-2 justify-content-center">
                                                <a href="<?= Url::to(['terima', 'id' => $model['id'] ?? '']) ?>" class="text-decoration-none" data-bs-toggle="tooltip" target="_blank" data-bs-title="Terima Berkas">
                                                    <i class="material-symbols-outlined fs-18 text-info text-success">check</i>
                                                </a>
                                                <a href="<?= Url::to(['tolak', 'id' => $model['id'] ?? '']) ?>" class="text-decoration-none" data-bs-toggle="tooltip" target="_blank" data-bs-title="Tolak Berkas">
                                                    <i class="material-symbols-outlined fs-18 text-info text-danger">block</i>
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
...
<?php
$autocompleteUrl = Url::to(['search-diterima']);
$js = <<<JS
var tooltipTriggerList = [].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]'))
var tooltipList = tooltipTriggerList.map(function (tooltipTriggerEl) {
  return new bootstrap.Tooltip(tooltipTriggerEl)
})
const searchInput = document.getElementById('search_diterima');
const resultsBox = document.getElementById('autocomplete-results');
const searchForm = document.getElementById('form-search-diterima');
const autocompleteUrl = '{$autocompleteUrl}';
// ... (Fungsi JS Autocomplete sama seperti sebelumnya) ...
