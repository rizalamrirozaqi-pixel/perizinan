...
/** @var array $jenisIzinList */

$this->title = 'Referensi Jenis Permohonan';
    ...
    <div class="card bg-white border-0 rounded-3 mb-4">
        <div class="card-body p-4">
            ...
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-lg-4 mb-3">
                <?= Html::beginForm(['index'], 'get', ['id' => 'form-search-permohonan', 'class' => 'table-src-form align-items-center']) ?>
                <div class="col-md-5 position-relative">
                    ...
                    <div id="autocomplete-container" class="position-relative" style="width: 400px;">
                        <?= Html::textInput('search', Yii::$app->request->get('search'), [
                            'id' => 'search_permohonan',
                            'class' => 'form-control',
                            'style' => 'padding-left: 3rem; width: 400px;',
                            'placeholder' => 'Cari Nama Permohonan...',
                            'autocomplete' => 'off'
                        ]) ?>
                    ...
                    <table class="table align-middle">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Jenis Izin</th>
                                <th>Nama Jenis Izin Permohonan</th>
                                <th>Register</th>
                                <th>Status</th>
                                <th>Aksi</th>
                            </tr>
                        ...
                        <tbody>
                            ...
                            <?php foreach ($models as $index => $model): ?>
                                <tr>
                                    ...
                                    <td class="text-secondary"><?= Html::encode($model['nama_izin_display']) ?></td>
                                    <td class="text-secondary"><?= Html::encode($model['nama_permohonan']) ?></td>
                                    <td class="text-secondary"><?= Html::encode($model['register']) ?></td>
                                    <td>
...
<div class="modal fade" id="modalFormPermohonan" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fs-5" id="modalLabel">Tambah Jenis Permohonan</h5>
            ...
            <div class="modal-body p-4">
                ...
                <div class="mb-3">
                    <label class="form-label fw-bold text-secondary">Jenis Izin <span class="text-danger">*</span></label>
                    <select class="form-select form-control" name="DynamicModel[id_jenis_izin]" id="selectJenisIzin" required>
                        <option value="">Silahkan pilih</option>
                        <?php foreach ($jenisIzinList as $id => $nama): ?>
                ...
                <div class="mb-3">
                    <label class="form-label fw-medium text-secondary">Nama Jenis Izin Permohonan</label>
                    <input type="text" name="DynamicModel[nama_permohonan]" id="inputNamaPermohonan" class="form-control" required>
                </div>

                <div class="mb-3">
                    <label class="form-label fw-medium text-secondary">Register</label>
                    <input type="text" name="DynamicModel[register]" id="inputRegister" class="form-control">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="DynamicModel[is_aktif]" id="statusF" value="F">
...
$urlAutocomplete = Url::to(['search-permohonan']);
...
var myModal = new bootstrap.Modal(document.getElementById('modalFormPermohonan'));
var allData = {$jsonData};

// --- TAMBAH DATA ---
$('.btn-add-modal').on('click', function() {
    $('#modalLabel').text('Jenis Izin Permohonan, Tambah catatan baru');
    $('#dynamic-form').attr('action', '{$urlCreate}');
    $('#dynamic-form')[0].reset();
    
    // Default Values
    $('#statusT').prop('checked', true); // Default Aktif
...
$('.btn-edit-modal').on('click', function() {
    ...
    if(item) {
        $('#modalLabel').text('Edit Jenis Izin Permohonan');
        ...
        // Isi Form
        $('#selectJenisIzin').val(item.id_jenis_izin); // Set Dropdown Value
        $('#inputNamaPermohonan').val(item.nama_permohonan);
        $('#inputRegister').val(item.register);
...
// --- AUTOCOMPLETE ---
const searchInput = document.getElementById('search_permohonan');
const resultsBox = document.getElementById('autocomplete-results');
const searchForm = document.getElementById('form-search-permohonan');
